    ...
    return ( 
        <div className="bg-bg flex justify-center w-full flex-row text-textMain">
            <div className="flex flex-row mx-7 bg-bg w-full justify-between">
                <button className="text-textMain my-5 h-11 w-12 rounded-3xl backdrop-brightness-110 hover:scale-110 hover:cursor-pointer transition-transform duration-200 hover:backdrop-brightness-200 z-50" onClick={toggleAddingWorkout}>{addingWorkout ? '-' : '+'}</button>
                <div className="my-2 w-full h-fit z-10 bg-bg text-center rounded-lg">
                    <button onClick={toggleDropdown} className="dropdown-toggle">
                        ...
                        {isOpen ? <ChevronUpIcon className="size-6 text-textMain inline p-1 bg-bg" /> : <ChevronDownIcon className="size-6 text-textMain inline p-1 bg-bg" />}
                    </button>
                    {isOpen ? 
                        ((splits.length > 0) ? 
